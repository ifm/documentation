...
void ItemCallback(ifm3d::Frame::Ptr frame) {
    if (frame->HasBuffer(ifm3d::buffer_id::O3R_RESULT_JSON)){
        ...
        std::cout << "Detected item(s): " 
                  << result_json["getItem"]["item"]
                  << std::endl;
    }
}
...
int main(){
    ...
    fg->OnNewFrame(&ItemCallback);

    ////////////////////////////////
    // Trigger the application
    ////////////////////////////////
    std::this_thread::sleep_for(2s);
    ifm3d::json getItem_parameters = {
        {"depthHint", 1.2},
        {"itemIndex", 0}
    };
    ifm3d::json getItem_command = {
            {"applications",{
                {"instances",{
                    {APP_PORT,{
                        {"configuration", {
